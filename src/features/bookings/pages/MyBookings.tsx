                ...
                `}>
                  <div className="h-32 bg-slate-800 relative">
                    ...
                    <div className="absolute top-3 right-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide shadow-sm backdrop-blur-md ${isCancelled
                          ? 'bg-red-900/80 text-red-100 border border-red-500/30'
                          : booking.status === 'pending'
                            ? 'bg-yellow-900/80 text-yellow-200 border border-yellow-500/30'
                            : isPast
                              ? 'bg-slate-700/80 text-slate-300 border border-slate-600'
                              : 'bg-emerald-900/80 text-emerald-100 border border-emerald-500/30'
                        }`}>
                        {isCancelled ? 'Cancelled' : booking.status === 'pending' ? 'Pending' : isPast ? 'Completed' : 'Confirmed'}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-700 flex gap-3">
                      {!isCancelled && !isPast && (
                        <button
                          ...
                          disabled={!canCancel}
                          className={`flex-1 py-2 px-3 rounded-xl text-sm font-bold transition-colors border ${canCancel
                      ...
                      <Link
                        to={`/turfs/${turf?.id}`}
                        className={`text-center py-2 px-3 rounded-xl text-sm font-bold transition-colors border border-transparent shadow-lg shadow-emerald-500/20 ${!isCancelled && !isPast

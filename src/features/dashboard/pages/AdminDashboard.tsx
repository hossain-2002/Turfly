                        ...
                        <div className="block md:hidden">
                            {filteredBookings.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-16 px-6">
                                    ...
                                    <p className="text-slate-500 text-sm mt-1 text-center">No bookings found matching your criteria.</p>
                                </div>
                            ) : (
                                        ...
                                        return (
                                            <div key={booking.id} className="p-4 space-y-3">
                                                ...
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                                        <div className="h-9 w-9 rounded-full bg-slate-800 flex items-center justify-center text-emerald-400 font-bold border border-slate-700 flex-shrink-0">
                                                            {booking.guestName ? booking.guestName.charAt(0) : <User className="w-4 h-4" />}
                                                        </div>
                                                        <div>
                                                            <div className="text-sm font-bold text-white">{booking.guestName || 'Unknown'}</div>
                                                            <div className="text-xs text-slate-400 flex items-center gap-1">
                                                                <Phone className="w-3 h-3" /> {booking.guestContact || 'N/A'}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <span className={`px-2.5 py-1 text-xs font-bold rounded-full uppercase border ${booking.status === 'confirmed' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' :
                                                            booking.status === 'pending' ? 'bg-yellow-900/30 text-yellow-400 border-yellow-500/30' :
                                                                'bg-red-900/30 text-red-400 border-red-500/30'
                                                        }`}>{booking.status}</span>
                                                </div>

                                                {/* Middle row: turf + date + amount */}
                                                <div className="grid grid-cols-2 gap-2 text-xs text-slate-400">
                                                    <div>
                                                        <span className="text-slate-500 uppercase tracking-wide font-bold text-[10px] block mb-0.5">Turf</span>
                                                        <div className="text-slate-200 font-medium">{turf?.name}</div>
                                                        <div className="flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3" />{turf?.location}</div>
                                                    </div>
                                                    <div>
                                                        <span className="text-slate-500 uppercase tracking-wide font-bold text-[10px] block mb-0.5">Date & Time</span>
                                                        <div className="flex items-center gap-1"><Calendar className="w-3 h-3" />{booking.date}</div>
                                                        <div className="flex items-center gap-1 mt-0.5"><Clock className="w-3 h-3" />{formatTime(booking.startTime)} – {formatTime(booking.startTime + booking.duration)}</div>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className="flex gap-2">
                                                        {booking.status === 'pending' && (
                                                            <button
                                                                ...
                                                                className="text-white bg-emerald-600 hover:bg-emerald-500 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center border border-emerald-500 transition-colors"
                                                            >
                                                        ...
                                                        {booking.status !== 'cancelled' && (
                                                            <button
                                                                ...
                                                                className="text-red-400 hover:bg-red-900/30 hover:text-red-300 px-3 py-1.5 rounded-lg text-xs font-bold border border-transparent hover:border-red-900/50 transition-colors"
                                                            >
                                                                Cancel
                                                            </button>
                                                        )}
                                        ...
                                        <tr>
                                            <td colSpan={6} className="px-6 py-20 text-center">
                                                <div className="flex flex-col items-center justify-center">
                                                    ...
                                                    <h3 className="text-lg font-medium text-white">No bookings found</h3>
                                                    <p className="text-slate-500 text-sm mt-1">No bookings found matching your criteria.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                        filteredBookings.map((booking) => {
                                            ...
                                            return (
                                                <tr key={booking.id} className="hover:bg-slate-800/50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full uppercase tracking-wide border ${booking.status === 'confirmed' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' :
                                                                booking.status === 'pending' ? 'bg-yellow-900/30 text-yellow-400 border-yellow-500/30' :
                                                                    'bg-red-900/30 text-red-400 border-red-500/30'
                                                            }`}>
                                                            {booking.status}

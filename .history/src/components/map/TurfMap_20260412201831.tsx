...
const TurfMap: React.FC<TurfMapProps> = ({ turfLocation, turfName }) => {
    ...
    // Store the initial map center on mount
    useEffect(() => {
        if (mapRef.current && defaultCenterRef.current === null) {
    ...
    // Handle "Go to My Location" button toggle
    const handleGoToMyLocation = () => {
        ...
        if (isZoomedToUser) {
            ...
            if (defaultCenterRef.current) {
                map.flyTo(
                    [defaultCenterRef.current.lat, defaultCenterRef.current.lng],

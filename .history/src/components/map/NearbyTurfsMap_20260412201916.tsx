...
const NearbyTurfsMap: React.FC<NearbyTurfsMapProps> = ({ turfs, userLocation }) => {
  ...
  const [mapDefaultState, setMapDefaultState] = useState<{
    center: [number, number];
    zoom: number;
  } | null>(null);

  const handleMapReady = (defaultCenter: [number, number], defaultZoom: number) => {
    setMapDefaultState({ center: defaultCenter, zoom: defaultZoom });
  };

  const handleGoToMyLocation = () => {
    if (!userLocation) return;
    setIsZoomedToUser(!isZoomedToUser);
  };
  ...
  return (
    <div className="relative w-full rounded-xl overflow-hidden shadow-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800">
      ...
      <button
        ...
        className={`absolute bottom-4 right-4 z-[1000] p-3 rounded-full shadow-lg transition-all duration-200 ${
          userLocation
            ? `${
              isZoomedToUser
        ...
        title={userLocation ? (isZoomedToUser ? 'Reset map view' : 'Go to my location') : 'Location not available'}
      >
        <Navigation
      ...
      >
        ...
        <MapController userLocation={userLocation} turfs={turfs} isZoomedToUser={isZoomedToUser} onMapReady={handleMapReady} toggleGoToMyLocation={isZoomedToUser} />

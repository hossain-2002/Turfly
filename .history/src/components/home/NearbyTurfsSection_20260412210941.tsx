...
import { MapPin, Navigation, Loader2, MapPinOff, Locate } from 'lucide-react';
...
const LocateMeControl: React.FC<{
  ...
  const defaultCenterRef = useRef<L.LatLng | null>(null);
  const defaultZoomRef = useRef<number | null>(null);

  useEffect(() => {
    if (!defaultCenterRef.current) {
      defaultCenterRef.current = map.getCenter();
      defaultZoomRef.current = map.getZoom();
    }
  }, [map]);
  ...
  const handleClick = useCallback(() => {
    ...
    const resetCenter = defaultCenterRef.current;
    const resetZoom = defaultZoomRef.current ?? 12;
    if (resetCenter) {
      map.flyTo([resetCenter.lat, resetCenter.lng], resetZoom, { duration: 1.2 });
    }
    setIsZoomedToUser(false);
  }, [isZoomedToUser, map, userLocation]);
  ...
  return (
    <div className="absolute bottom-5 right-4 z-[1000]">
      <button
        onClick={handleClick}
        title={isZoomedToUser ? 'Zoom out map' : 'Go to my location'}
        aria-pressed={isZoomedToUser}
        className="
          w-10 h-10 bg-white dark:bg-slate-800 rounded-lg shadow-lg

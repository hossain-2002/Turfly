...
const HoverMarker: React.FC<{
  ...
  if (!turf.coordinates) {
    console.warn(`⚠️ Turf ${turf.id} missing coordinates, skipping marker`);
    return null;
  }
      ...
      eventHandlers={{
        mouseover: () => {
          try {
            ...
            console.error(`❌ Error opening popup for ${turf.id}:`, error);
          }
        },
        mouseout: () => {
          try {
            ...
            console.error(`❌ Error closing popup for ${turf.id}:`, error);
          }
        },
      }}
    >
...
const NearbyTurfsSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const cached = getCachedLocation();
    if (cached) {
      console.log('✅ Using cached user location:', cached);
      return;
    }

    if (!('geolocation' in navigator)) {
      console.warn('⚠️ Geolocation API not available in this browser');
      setGeoStatus('denied');
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        console.log('✅ User location acquired:', loc);
        setUserLocation(loc);
  ...
  const turfsWithDistance = useMemo(() => {
    ...
    console.log(`📍 Computed ${withDist.length} turfs with distances`, withDist);
    return withDist;
  }, [turfs, userLocation]);

  // Nearest 4 for the map focus + quick cards
  const nearbyTurfs = useMemo(() => {
    const nearby = turfsWithDistance.slice(0, 4);
    console.log(`🎯 Nearest turfs (4): ${nearby.length} items`);
    return nearby;
  }, [turfsWithDistance]);
